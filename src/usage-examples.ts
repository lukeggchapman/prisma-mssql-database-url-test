...
async function exampleDatabaseUrl() {
  ...
  try {
    ...
    const result = await prisma.$queryRaw`SELECT 'DATABASE_URL approach works!' as message`;
...
async function exampleIndividualConfig() {
  ...
  try {
    ...
    await prisma.$connect();
    const result = await prisma.$queryRaw`SELECT 'Individual config approach works!' as message`;
    console.log('✅ Success:', result);
...
async function exampleMultipleScenarios() {
  ...
  const testPasswords = [
    ...
    { name: 'URL Characters', password: 'Pass@#%&2024', port: 1435 },
  ...
  for (const test of testPasswords) {
    ...
    try {
      const config = createMssqlConfig('localhost', test.port, 'sa', test.password);
      const adapter = await createMssqlAdapterFromConfig(config);
      const prisma = new PrismaClient({ adapter });
      ...
      console.log('❌ Individual config: FAILED -', error instanceof Error ? error.message : 'Unknown error');
    }
    
    // Try DATABASE_URL approach
    try {
      const databaseUrl = createDatabaseUrl('localhost', test.port, 'sa', test.password);
      const adapter = await createMssqlAdapterFromUrl(databaseUrl);
      const prisma = new PrismaClient({ adapter });
      ...
      console.log('❌ DATABASE_URL: FAILED -', error instanceof Error ? error.message : 'Unknown error');
    }
  }
}
...
function examplePrismaCli() {
  ...
  console.log('The prisma.config.ts file now supports environment-based configuration:');
  console.log('');
  ...
  console.log('DATABASE_URL="sqlserver://sa:MyPass%7B123%7D@localhost:1433;database=test" npx prisma db push');
  console.log('');

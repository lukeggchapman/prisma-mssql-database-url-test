...
const testPasswords: TestPassword[] = [
  ...
  },
  {
    ...
    raw: "Pass@#%&(Test)2024",
    port: 1435,
    expectedEscaping: "Pass@#%&(Test)2024", // These chars don't get escaped by our pattern
  },
  {
...
async function testCliOperation(password: TestPassword): Promise<{
  ...
  const connectionString = createCliDatabaseUrl(
    "localhost",
    password.port,
    "sa",
    password.raw,
  ...
  console.log(
    `   ✅ Escaping ${escapingMatches ? "matches" : "DIFFERS FROM"} expected`
  );
...
export async function runCliVsAdapterComparison(): Promise<void> {
  ...
  results.forEach((result) => {
    ...
    const comparison =
      result.cliSuccess === result.adapterSuccess
        ? result.cliSuccess
          ? "🎉 Both Work"
          : "⚠️ Both Fail"
        : "🔄 Different Results";
    ...
    console.log(`\n${result.password}:`);
    console.log(
      `  CLI:     ${cliStatus} ${result.cliSuccess ? "SUCCESS" : "FAILED"}`
    );
    console.log(
      `  Adapter: ${adapterStatus} ${
        result.adapterSuccess ? "SUCCESS" : "FAILED"
      }`
    );
    ...
    if (result.adapterError)
      console.log(`  Adapter Error: ${result.adapterError}`);
  ...
  console.log(`\n🎯 Summary:`);
  console.log(
    `   CLI Success Rate:     ${cliSuccessCount}/${
      results.length
    } (${Math.round((cliSuccessCount / results.length) * 100)}%)`
  );
  console.log(
    `   Adapter Success Rate: ${adapterSuccessCount}/${
      results.length
    } (${Math.round((adapterSuccessCount / results.length) * 100)}%)`
  );
  console.log(
    `   Both Working:         ${bothSuccessCount}/${
      results.length
    } (${Math.round((bothSuccessCount / results.length) * 100)}%)`
  );
  ...
  if (
    cliSuccessCount === adapterSuccessCount &&
    cliSuccessCount === results.length
  ) {
    console.log(
      "   🎉 Both approaches work equally well with proper escaping!"
    );
  } else if (adapterSuccessCount > cliSuccessCount) {
    console.log(
      "   📈 Adapter approach is more reliable than CLI with escaping"
    );
  } else if (cliSuccessCount > adapterSuccessCount) {
    console.log(
      "   📈 CLI approach with escaping is more reliable than adapter"
    );
  } else {
    console.log(
      "   🤔 Mixed results - some passwords work better with different approaches"
    );
  }
}
